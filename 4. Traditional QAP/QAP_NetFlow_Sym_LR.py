...
class Branch :  # GLB
    ...
    def Initialization(self) :
        ...
        #print(self.init_assignment_mat[k_ind - 1, i_ind - 1])
...
def g_Initialization() :
    ...
    for l in range(g_nb_of_links) :
        if g_link_list[l].link_type == 1 :  # transportation link
    ...
    print('reading', g_nb_of_branch, 'branches')

    for solution in pool.map(GLB, input_list) :
        #print('got a solution of', solution['branch_seq'])
...
def local_search():
    global g_assignment_mat
    UpperBound = np.sum(g_flow_mat * np.matmul(np.matmul(g_assignment_mat, g_trans_cost_mat), g_assignment_mat.T)) + np.sum(g_build_cost_orig_mat * g_assignment_mat)
    local_search_list=deque()
    local_search_list.append(g_assignment_mat)
    ...
    while (len(local_search_list)!=0 and Flag_Swap<=10000):
        ...
        for i in range(nb_local):
            ...
            tmp_UB = np.sum(
                g_flow_mat * np.matmul(np.matmul(temp_assign_mat, g_trans_cost_mat), temp_assign_mat.T)) + np.sum(
                g_build_cost_orig_mat * temp_assign_mat)
   ...
   #g_assignment_mat = g_assignment_mat_tmp

    return UpperBound, g_assignment_mat
...
if "__main__" == __name__ :
    ...
    for iter in range(nb_iter):
        g_sub_gradient_mat = np.zeros([g_nb_of_dest_location, g_nb_of_dest_building])
        g_assignment_mat = np.zeros([g_nb_of_orig_building, g_nb_of_orig_location])
        # 【0】 GLB sub problem
        lower_solution_1 = Hungarian_1(g_GLB_cost_mat+g_build_cost_orig_mat+g_lag_multiplier_mat.T*g_nb_of_dest_location)
        assignment_count_mat = Trace(lower_solution_1)
        for l in range(g_nb_of_orig_location):
                k_ind =lower_solution_1['location_ind'][l]
                i_ind =lower_solution_1['building_ind'][l]
                g_assignment_mat[i_ind][k_ind] = 1
        ...
        for l in range(g_nb_of_dest_location):
                l_ind =lower_solution_1['location_ind'][l]
                j_ind =lower_solution_1['building_ind'][l]
                g_sub_gradient_mat[l_ind][j_ind] = g_nb_of_dest_location
        LowerBound = lower_solution_1['value']
        UpperBound = np.sum(g_flow_mat*np.matmul(np.matmul(g_assignment_mat,g_trans_cost_mat),g_assignment_mat.T))+np.sum(g_build_cost_orig_mat*g_assignment_mat)
        #UpperBound, g_assignment_mat=local_search()
        ...
        if UpperBound <= UB:
            UB = UpperBound
            best_solution = g_assignment_mat

        print(iter, 'iteration: Upper bound = ', UB)
        print(iter, 'iteration: Lower bound = ', LB)
