...
UB=100000000000 #float(np.inf)
LB=float(-np.inf)
nb_local = 1000
nb_iter = 1
step_size=0.5
M = 1000000 # Important parameter in symmetric case
...
class Branch :  # it is the branch for Gilmore Lawler bound
    ...
    def Initialization(self) :
        ...
        self.init_assignment_mat = np.matmul(g_trans_cost_mat[k_ind - 1 :k_ind].T, g_flow_mat[i_ind - 1 :i_ind]) # do not consider the building cost in l-j side
...
class TreeNode: # This is the node for tree serch
    def __init__(self, builidng_id, location_id, tree_vec, parent) :
        ...
        self.Initialization()
        # e.g. [-1,0,1,2] means the location 1 is assigned by building 0; location 2 is assigned by building 1
        # if tree node building id =3, location id = 4,  then tree_vec[3]=2  as the tree is constructed according to the order of building_id, it is in layer 5 (i.e. root 0,1,2,3)
    def Initialization(self) :
        global g_nb_of_tree_node
        self.tree_node_id = g_nb_of_tree_node
        g_nb_of_tree_node += 1
        #print('generate',g_nb_of_tree_node,'tree nodes')
...
def g_Initialization() :
    ...
    for l in range(g_nb_of_links) :
        if g_link_list[l].link_type == 2 :  # 'building_orig'
            ...
            #print(branch.init_assignment_mat)
...
def Branch_update(multiplier_mat,tmp_value):
    # multiplier_mat = np.zeros([g_nb_of_dest_location, g_nb_of_dest_building])
    # tmp_value = 0
    # for k in range(len(tree_vec)) :
    #     if tree_vec[k] != -1 :
    #         l_ind = k
    #         j_ind = tree_vec[k]
    #         multiplier_mat[l_ind, j_ind] = M
    #         tmp_value = tmp_value + M
    input_list = []
    for b in range(g_nb_of_branch):
        #print(b,'...',g_branch_list[b].init_assignment_mat)
        g_branch_list[b].assignment_mat = g_branch_list[b].init_assignment_mat-multiplier_mat
    ...
    for solution in pool.map(GLB, input_list) :
        ...
        g_branch_list[solution['branch_seq']].lower_value = solution['value'] + tmp_value
...
def local_search():
    ...
    UpperBound = np.sum(
        g_flow_mat * np.matmul(np.matmul(g_assignment_mat_1, g_trans_cost_mat), g_assignment_mat_2)) + np.sum(
    ...
    while (len(local_search_list)!=0 and Flag_Swap<=10000):
        ...
        for i in range(nb_local):
            ...
            tmp_UB = np.sum(g_flow_mat * np.matmul(np.matmul(temp_assign_mat, g_trans_cost_mat), g_assignment_mat_2)) + np.sum(g_build_cost_orig_mat * temp_assign_mat) + np.sum(g_build_cost_dest_mat * g_assignment_mat_2)
...
if "__main__" == __name__ :
    ...
    g_ReadInputData()
    g_Initialization()
    # The first time lower and upper bound
    print('Calculate GLB...')
    t0 = datetime.datetime.now()
    ...
    lower_solution_2 = Hungarian_2(g_build_cost_dest_mat)
    ...
    # UpperBound=0
    # for i in range(g_nb_of_orig_building):
    #     for k in range(g_nb_of_orig_location):
    #         for l in range(g_nb_of_dest_location):
    #             for j in range(g_nb_of_dest_building):
    #                 tmp=g_assignment_mat_1[i][k]*g_flow_mat[i][j]*g_trans_cost_mat[k][l]*g_assignment_mat_2[l][j]
    #                 UpperBound+=tmp
    ...
    if UpperBound <= UB:
        ...
        print(best_solution_1)
        best_solution_2 = g_assignment_mat_2
    GAP = np.abs(UB - LB)/ UB
    print('GLB - Lower bound = ', LB)
    print('GLB - Upper bound = ', UB)
    print('GAP = ', GAP)


    # Finds minimum cost using Branch and Bound.
    # Step 1:  Create a priority queue (heap) to store live nodes of search tree;
    pq = priority_queue()

    # Step 2: Initailize heap to dummy root tree node with cost 0
    assigned = np.zeros(g_nb_of_dest_location) # in our problem g_nb_of_dest_building=g_nb_of_dest_location
    tree_vec = -np.ones(g_nb_of_dest_building,dtype=int)
    root = TreeNode(-1,-1,tree_vec,-1) # generate the root tree_node
    root.lower_value = LB
    # Add dummy node to list of live nodes;
    pq.put((root.lower_value,root.tree_node_id,root))


    #Finds a live node with least cost, add its childrens to list of live nodes and finally deletes it
    iter = 0
    t2 = datetime.datetime.now()
    print (iter, 'iter: using time ', t2-t0)
    while (pq.queue):
        ...
        for l in range(g_nb_of_dest_location):
            #print(live_node.tree_vec)
            tmp_tree_vec = live_node.tree_vec.copy() # should copy, not use ip address
            if tmp_tree_vec[l] == -1 and list(tmp_tree_vec).count(-1) > 1:
                tmp_tree_vec[l] = live_building_id
                #print(tmp_tree_vec)
                tree_node = TreeNode(live_building_id, l, tmp_tree_vec, live_node)
                multiplier_mat = np.zeros([g_nb_of_dest_location, g_nb_of_dest_building])
                tmp_value = 0
                for k in range(len(tree_node.tree_vec)) :
                ...
                g_assignment_mat_2 = np.zeros([g_nb_of_dest_location,g_nb_of_dest_building])
                lower_solution_1 = Hungarian_1(g_GLB_cost_mat + g_build_cost_orig_mat)
                for ll in range(g_nb_of_orig_location):
                        k_ind =lower_solution_1['location_ind'][ll]
                        i_ind =lower_solution_1['building_ind'][ll]
                        g_assignment_mat_1[i_ind][k_ind] = 1

                lower_solution_2 = Hungarian_2(g_build_cost_dest_mat-multiplier_mat)
                for ll in range(g_nb_of_dest_location):
                        l_ind =lower_solution_2['location_ind'][ll]
                        j_ind =lower_solution_2['building_ind'][ll]
                        # print('a',l_ind)
                        # print('b',j_ind)
                        g_assignment_mat_2[l_ind][j_ind] = 1
                        # print('c',g_assignment_mat_2)
                # print('the lead assignment has ',lower_solution_1['value'])
                # print('the follower assignment has ', lower_solution_2['value']+tmp_value)
                tree_node.lower_value = lower_solution_1['value'] + lower_solution_2['value']+tmp_value
                print('searching tree node : building', 4000+live_building_id+1, 'location', 3000+l+1, 'with lower value', tree_node.lower_value)
                UpperBound = np.sum(g_flow_mat * np.matmul(np.matmul(g_assignment_mat_1, g_trans_cost_mat),
                                                            g_assignment_mat_2)) + np.sum(g_build_cost_orig_mat * g_assignment_mat_1) + np.sum(g_build_cost_dest_mat * g_assignment_mat_2)
                ...
                UpperBound, g_assignment_mat_1, g_assignment_mat_2 = local_search()
                # if list(tmp_tree_vec).count(-1) == 1:
                #     #print(1)
                ...
                if UpperBound <= UB:
                    ...
                    best_solution_1 = g_assignment_mat_1
                    best_solution_2 = g_assignment_mat_2
                if list(tmp_tree_vec).count(-1)>1: # as still two locations are not assigned, the solution is an lower bound solution
                    if (tree_node.lower_value <= UB) and (tree_node.lower_value>=LB):
                        pq.put((tree_node.lower_value,tree_node.tree_node_id,tree_node))
        iter += 1
        t1 = datetime.datetime.now()
